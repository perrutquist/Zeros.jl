...
Base.convert(::Type{T}, x::MyComplex) where {T<:Real} = x.im == 0 ? T(x.re) : throw(InexactError(Symbol(T), T, x))
Base.convert(::Type{Complex{T}}, x::MyComplex) where {T<:Real} = Complex(T(x.re), T(x.im))
...
for op in (:+, :-, :*, :/)
    @eval begin
        Base.$op(x::Real, y::MyComplex) = $op(MyComplex(x, Zero()), y)
        Base.$op(x::MyComplex, y::Real) = $op(x, MyComplex(y, Zero()))
    end
end
